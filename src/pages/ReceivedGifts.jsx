...
export default function ReceivedGifts({ tenant, onBack, showModal }) {
  ...
  const handleRemove = async (id) => {
    showModal({
      ...
      message:
        "Tem certeza que deseja remover a reserva deste presente? Ele voltará para a lista pública e ficará disponível para outros convidados.",
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-text-secondary mt-1">
                Gerencie os presentes reservados pelos convidados
              </p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl shadow-soft border border-cream-dark">
            ...
            <p className="text-text-secondary max-w-md mx-auto">
              Quando os convidados escolherem os presentes, eles aparecerão
              aqui.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {gifts.map((gift) => (
              <div
                key={gift.id}
                className="bg-white rounded-2xl p-6 shadow-soft border border-cream-dark flex flex-col gap-4"
              >
                {gift.imagem_url && (
