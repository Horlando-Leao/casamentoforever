...
import { CATEGORIAS, DEFAULT_GIFTS } from "../lib/defaultGifts";
...
function DefaultGiftsModal({ onSelect, onClose }) {
  ...
  const filtrados = DEFAULT_GIFTS.filter((g) => {
    const matchCategoria =
      categoriaAtiva === "Todos" || g.categoria === categoriaAtiva;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-5 border-b border-gray-100 shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-500 hidden sm:block mt-0.5">
              Selecione para pré-preencher o formulário
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div
          className="overflow-y-auto flex-1 px-4 sm:px-6 pb-3"
        ...
        <div className="hidden sm:block px-6 py-3 border-t border-gray-100 bg-gray-50 rounded-b-3xl shrink-0">
          <p className="text-xs text-center text-gray-400">
            Clique em um presente para pré-preencher o formulário. Você poderá
          ...
          >
            ...
            <div className="text-left">
              <p className="text-sm font-bold leading-tight">
                Escolher Presente Sugerido
              </p>
              <p className="text-xs font-normal text-yellow-100">
                Pré-preenche o formulário com um clique
              </p>
            </div>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5 ml-auto opacity-80 group-hover:translate-x-1 transition-transform"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        >
          ...
          <div>
            ...
            {sites.map((site, index) => (
              <div
                key={index}
                className="mb-4 p-4 border border-gold border-opacity-20 rounded-xl bg-cream/50"
              >
                <div className="grid grid-cols-1 gap-3 mb-3">
                  <input
                    ...
                    value={site.label}
                    onChange={(e) =>
                      handleUpdateSite(index, "label", e.target.value)
                    }
                  ...
                  <input
                    ...
                    value={site.url}
                    onChange={(e) =>
                      handleUpdateSite(index, "url", e.target.value)
                    }
                    placeholder="https://..."
                    className="px-3 py-2 border border-gold border-opacity-30 rounded-lg focus:ring-2 focus:ring-gold focus:border-transparent outline-none text-sm"
                  />
