...
export default function PublicEventDetail({ qrToken }) {
  ...
  const submitReserve = async (e) => {
    ...
    try {
      ...
      setGifts(
        gifts.map((g) =>
          g.id === data.gift.id ? { ...g, reserved: true } : g,
        ),
      );
    } catch (err) {
    ...
    <div className="min-h-screen bg-cream selection:bg-rose-gold/20 relative">
        ...
        <div className="relative max-w-4xl mx-auto px-6 py-16 md:py-24 text-center">
          <div className="flex flex-col items-center gap-4 mb-6">
            ...
            {invitation?.event?.data_evento && (
              <Countdown
                date={invitation.event.data_evento}
                time={invitation.event.horario}
              />
            )}
          </div>
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-display text-text-primary mb-6 leading-tight">
            {invitation?.couple?.nome1}{" "}
            <span className="text-rose-gold italic">&</span>{" "}
            {invitation?.couple?.nome2}
          </h1>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 -mt-8 relative z-10">
                        ...
                        <div className="flex-1">
                          ...
                          {invitation.event.data_evento && (
                            <p className="text-lg font-semibold text-text-primary capitalize mb-1">
                              {new Date(
                                invitation.event.data_evento + "T00:00:00",
        ...
        <div ref={giftsRef} className="scroll-mt-8">
          <div className="mb-12">
            <h2 className="text-4xl font-display text-text-primary mb-3">
              💝 Lista de Presentes
            </h2>
            <p className="text-text-secondary">
              Escolha um presente para os noivos com muito carinho!
            </p>
          </div>
          ...
          {giftsLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
              {gifts.map((gift, index) => (
                ...
                >
                  <div
                    className={`relative w-full aspect-[4/3] bg-cream-alt flex items-center justify-center overflow-hidden ${gift.reserved ? "grayscale opacity-75" : ""}`}
                  >
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    {gift.reserved ? (
                      <button
                        disabled
                        className="w-full py-3.5 bg-gray-100 text-gray-400 font-semibold rounded-xl cursor-not-allowed border border-gray-200 transition-colors"
                      >
                        Já Reservado
                      </button>
                    ) : (
      ...
      {selectedGift && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 z-[100] animate-fade-in">
          ...
          >
            ...
            <h2 className="text-2xl font-display text-text-primary mb-4 pr-8">
              Reservar: {selectedGift.nome}
            </h2>
            <button
            ...
            <p className="text-sm text-text-secondary mb-4 leading-relaxed">
              Para reservar este presente, precisamos de alguns dados. Assim
              garantimos que o presente seja único!
            </p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 z-[100] animate-fade-in">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl max-w-lg w-full p-6 sm:p-10 animate-slide-up-sheet sm:animate-fade-in text-center relative">
            ...
            <p className="text-text-secondary mb-8 text-lg">
              Você acabou de reservar <strong>{successGift.nome}</strong> para
              os noivos!
            </p>

            <div className="bg-cream-alt p-6 rounded-2xl mb-8 text-left border border-gold-light/30 shadow-inner">
              <h4 className="font-bold text-text-primary mb-4 text-center uppercase tracking-widest text-xs">
                Instruções para o Presente
              </h4>

              {successGift.sites && successGift.sites.length > 0 ? (
                <div>
                  <p className="text-xs font-bold text-text-secondary mb-3 uppercase">
                    Onde Comprar
                  </p>
                  <div className="space-y-3">
                    {successGift.sites.map((site, i) => (
                      ...
                      >
                        ...
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className="h-5 w-5 text-gold-light group-hover:text-gold"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                ...
                <div className="py-4 text-center">
                  <p className="text-sm text-text-light italic">
                    Para este presente, você pode realizar a compra em qualquer
