...
import { deleteGift, getGift, updateGift } from "../services/api";
...
export default function GiftDetail({
  ...
  const inputBase =
    "w-full px-4 py-2 border border-gold border-opacity-30 rounded-lg outline-none transition";
    ...
    <div className="min-h-screen bg-cream">
      <header className="bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-cream-dark shadow-sm">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <button
            onClick={onBack}
            className="flex items-center gap-2 text-gold-dark font-medium hover:text-rose-gold transition-colors p-2 -ml-2 rounded-lg hover:bg-cream-alt"
          >
            <svg
          ...
          {!isEditing && (
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 text-text-secondary hover:text-gold-dark hover:bg-cream-alt rounded-lg transition-colors"
                title="Editar"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 pb-24">
        {error && (
          <div className="mb-6 p-4 bg-red-50 border border-red-100 text-red-600 rounded-xl text-center flex items-center justify-center gap-2">
        ...
        <div className="bg-white rounded-3xl shadow-soft-lg overflow-hidden border border-cream-dark">
          ...
          <div className="p-6 sm:p-8 space-y-6">
            ...
            {isEditing && (
              <div>
                <label className="block text-sm font-bold text-text-secondary mb-2 tracking-wide uppercase">
                  Nome do Presente
                </label>
                <input
                  type="text"
            ...
            {/* Links */}
            <div>
              <label className="block text-sm font-bold text-text-secondary mb-3 tracking-wide uppercase">
                Links para compra
              </label>
              {sites.length === 0 && !isEditing && (
                <p className="text-text-light italic">
                  Nenhum link sugerido para compra.
                </p>
              )}
              {sites.map((site, i) => (
                <div
                  key={i}
                  className={`mb-3 ${isEditing ? "p-4 border border-cream-dark rounded-xl bg-gray-50/50" : ""}`}
                >
                  {isEditing ? (
                    <div className="grid grid-cols-1 gap-3 mb-3">
                      <input
                        ...
                        value={site.label}
                        onChange={(e) =>
                          handleUpdateSite(i, "label", e.target.value)
                        }
                      ...
                      <input
                        ...
                        value={site.url}
                        onChange={(e) =>
                          handleUpdateSite(i, "url", e.target.value)
                        }
                        placeholder="https://..."
                        className={siteInputClass}
                      />
                    ...
                    >
                      <span className="font-medium text-text-primary group-hover:text-gold-dark transition-colors">
                        {site.label}
                      </span>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="h-5 w-5 text-gold/50 group-hover:text-gold"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
