...
export function formatGiftWhatsAppMessage(gift) {
  ...
  parts.push(
    `Olá ${guestName}! Obrigado por reservar o presente *${gift.nome}*! 🎁`,
  );
  parts.push("");
...
export function buildGiftWhatsAppUrl(gift) {
  const rawPhone = gift.reserved_by_whatsapp
    ? String(gift.reserved_by_whatsapp).replace(/\D/g, "")
    : "";
...
export function formatInvitationWhatsAppMessage(event, names, publicUrl) {
  ...
  // Cabeçalho
  const nome1 = names?.nome1
    ? names.nome1.charAt(0).toUpperCase() + names.nome1.slice(1).toLowerCase()
    : "";
  const nome2 = names?.nome2
    ? names.nome2.charAt(0).toUpperCase() + names.nome2.slice(1).toLowerCase()
    : "";
  const coupleLabel =
    nome1 && nome2 ? `*${nome1} & ${nome2}*` : "*Nosso Casamento*";
  ...
  parts.push(
    `⏰ _Se organize e não deixe para a última hora! Reserve um tempo na sua agenda e escolha um presente para homenagear essa data especial._ 💕`,
  );
